...
uint16_t tlc_shiftUp(uint16_t zeroValue)
{
    ...
    while (p < tlc_GSData + NUM_TLCS * 36 - 1) {
        *(p - 1) = (*p << 4) | (*(p + 1) >> 4);
        *p = (*(p + 1) << 4) | (*(p + 2) >> 4);
        p += 2;
    }
    *(tlc_GSData + NUM_TLCS * 36 - 2) = (*(tlc_GSData + NUM_TLCS * 36 - 1) << 4)
                                      | ((zeroValue & 0x0F00) >> 8);
    *(tlc_GSData + NUM_TLCS * 36 - 1) = (uint8_t)zeroValue;
    return topValue;
}
...
{
    uint8_t *p = tlc_GSData + NUM_TLCS * 36 - 2;
    uint16_t zeroValue =
            ((uint16_t)(*(tlc_GSData + NUM_TLCS * 36 - 2) & 0x0F) << 8)
                      | *(tlc_GSData + NUM_TLCS * 36 - 1);
    while (p > tlc_GSData) {
        *(p + 1) = (*p >> 4) | (*(p - 1) << 4);
        *p = (*(p - 1) >> 4) | (*(p - 2) << 4);
        p -= 2;
